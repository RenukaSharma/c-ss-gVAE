...
class SSAD(object):
        ...
        for data in train_loader:
            ...
            if self.hybrid and self.ae_net is None:
                if dataset_name == 'malaria_dataset':
                    self.ae_net = build_autoencoder('malaria_net')
                elif dataset_name == 'nanofibre':
                    self.ae_net = build_autoencoder('nanofibre_vae')
                else:
                    raise ValueError(
                        'Hybrid SSAD without --load_ae is only supported for malaria_dataset and nanofibre.')
            if self.hybrid:
                *_, inputs = self.ae_net(inputs)
    ...
    def load_ae(self, dataset_name, model_path):
        ...
        if dataset_name == 'malaria_dataset':
            net_name = 'malaria_net'
        elif dataset_name == 'nanofibre':
            net_name = 'nanofibre_vae'
        else:
            raise ValueError(f'Unknown dataset_name for SSAD hybrid AE load: {dataset_name}')
