...
class DeepSADTrainer(BaseTrainer):
    ...
    def __init__(self, c, eta: float, dataset_name: str = 'cifar10', optimizer_name: str = 'adam', lr: float = 0.001,
                 ...
                 n_jobs_dataloader: int = 0, recon_param: float = 0.5, latent_param: float = 0.5, classifier_coeff: float = 1.0, eps: float = 0.1, tau: float = 0.1,
    ...
    def train(self, dataset: BaseADDataset = 'cifar10', net: BaseNet = None, dec_net: BaseNet = None,
        ...
        for epoch in range(self.n_epochs):
            ...
            for data in train_loader:
                ...
                if ablation_type == 'A':
                    losses, latent, dist_recon, reco_loss, _, _, _, _ = self.loss_A(data, ae_net, self.eps)
    ...
    def loss_A(self, data, ae_net, eps: float = 1e-5):
        ...
        inputs, labels, semi_targets = inputs.to(self.device), labels.to(self.device), semi_targets.to(self.device)

    
        outputs_encoded_mu, outputs_encoded_beta, outputs_recons_mu, outputs_recons_beta, sample, out_class = ae_net(inputs, ablation_type='A', eps=eps)
        ...
        # classifier_coeff = 1.0
        # import ipdb; ipdb.set_trace()
        labels= torch.FloatTensor(labels.cpu().float()).to(self.device)
        # import ipdb; ipdb.set_trace()
        classifier_term = - self.classifier_coeff * (labels*torch.log(out_class) + (1-labels)*torch.log(1-out_class))
                
        losses = latent_semisup + recon_semisup + classifier_term
        # losses = latent_semisup
        # losses = recon_semisup
        # losses = torch.where(semi_targets == 0, dist, self.eta * ((dist + self.eps) ** semi_targets.float()))
        recon_loss = self.criterion(inputs, outputs_recons_mu)
        losses = torch.mean(losses)
        return losses, latent_semisup, dist_recon, recon_semisup, outputs_encoded_mu, outputs_recons_mu, sample, out_class
    
    # def loss_A_woclassifier(self, data, ae_net, eps: float = 1e-5):
    #     inputs, labels, semi_targets, _ = data  # labels was _ previously
    #     inputs, semi_targets = inputs.to(self.device), semi_targets.to(self.device)

    #     outputs_encoded_mu, outputs_encoded_beta, outputs_recons_mu, outputs_recons_beta, sample = ae_net(inputs, ablation_type='A', eps=eps)
    ...
    def loss_B(self, data, ae_net):
        ...
        outputs_encoded_mu, outputs_encoded_beta, outputs_recons_mu, outputs_recons_beta, sample, _ = ae_net(inputs, ablation_type='B')
    ...
    def loss_C(self, data, ae_net):
        ...
        outputs_encoded_mu, outputs_encoded_beta, outputs_recons_mu, outputs_recons_beta, sample, _ = ae_net(inputs, ablation_type='C')
    ...
    def loss_C_alpha(self, data, ae_net):
        ...
        outputs_encoded_mu, outputs_encoded_alpha, outputs_encoded_beta, outputs_recons_mu, outputs_recons_alpha, outputs_recons_beta, sample, _ = ae_net(inputs, ablation_type='C_a')
        # outputs_encoded_alpha.detach()
        outputs_encoded_beta.detach()
        # outputs_recons_alpha.detach()
        outputs_recons_beta.detach()
    ...
    def loss_D(self, data, ae_net):
        ...
        outputs_encoded_mu, outputs_encoded_beta, outputs_recons_mu, outputs_recons_beta, sample, _ = ae_net(
    ...
    def loss_VAE(self, data, ae_net):
        ...
        outputs_encoded_mu, outputs_encoded_alpha, outputs_encoded_beta, outputs_recons_mu, outputs_recons_alpha, outputs_recons_beta, sample, _ = ae_net(
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                if self.ablation_type == 'A':
                    losses, latent, dist_recon, reco_loss, outputs_encoded_mu, outputs_recons_mu, sample, out_class = self.loss_A(data, ae_net)
                ...
                scores = torch.norm(sample, dim=1) ** 2 # DONE, best yet Rank #2
                # scores = torch.norm(outputs_encoded_mu, dim=1) ** 2 # DONE, Rank #3
                # scores = torch.norm((inputs.cpu() - outputs_recons_mu.cpu()).view(128, -1), dim=1) ##  Rank # 1
                # scores = torch.norm(outputs_recons_mu.view(128, -1), dim=1) ** 2 ##  Rank # 5
                # scores = out_class ## Rank # 4
